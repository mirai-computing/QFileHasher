...
/**
   ...
   @return CRYPT_OK if successful
*/
HASH_PROCESS(ltc_md4_process, md4_compress, md4, 64)
...
/**
   ...
   @return CRYPT_OK if successful
*/
int ltc_md4_done(hash_state * md, unsigned char *out)
{
    int i;
...
int ltc_md4_test(void)
{
    ...
    for(i = 0; i < (int)(sizeof(cases) / sizeof(cases[0])); i++) {
        ltc_md4_init(&md);
        ltc_md4_process(&md, (unsigned char *)cases[i].input, (unsigned long)strlen(cases[i].input));

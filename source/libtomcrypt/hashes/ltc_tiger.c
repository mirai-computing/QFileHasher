...
/**
   ...
   @return CRYPT_OK if successful
*/
HASH_PROCESS(ltc_tiger_process, tiger_compress, tiger, 64)
...
/**
   ...
   @return CRYPT_OK if successful
*/
int ltc_tiger_done(hash_state * md, unsigned char *out)
{
    LTC_ARGCHK(md  != NULL);
    LTC_ARGCHK(out != NULL);
...
int  ltc_tiger_test(void)
{
  ...
  for (i = 0; i < (int)(sizeof(tests) / sizeof(tests[0])); i++) {
      ltc_tiger_init(&md);
      ltc_tiger_process(&md, (unsigned char *)tests[i].msg, (unsigned long)strlen(tests[i].msg));
      ltc_tiger_done(&md, tmp);

...
/**
   ...
   @return CRYPT_OK if successful
*/
int ltc_sha384_done(hash_state * md, unsigned char *out)
{
...
int  ltc_sha384_test(void)
{
  ...
  for (i = 0; i < (int)(sizeof(tests) / sizeof(tests[0])); i++) {
      ltc_sha384_init(&md);
      ltc_sha384_process(&md, (unsigned char*)tests[i].msg, (unsigned long)strlen(tests[i].msg));

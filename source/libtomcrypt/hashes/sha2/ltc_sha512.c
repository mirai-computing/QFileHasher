...
/**
   ...
   @return CRYPT_OK if successful
*/
HASH_PROCESS(ltc_sha512_process, sha512_compress, sha512, 128)
...
/**
   ...
   @return CRYPT_OK if successful
*/
int ltc_sha512_done(hash_state * md, unsigned char *out)
{
    int i;
...
int  ltc_sha512_test(void)
{
  ...
  for (i = 0; i < (int)(sizeof(tests) / sizeof(tests[0])); i++) {
      ltc_sha512_init(&md);
      ltc_sha512_process(&md, (unsigned char *)tests[i].msg, (unsigned long)strlen(tests[i].msg));
